...
export default {
  data: () => ({
    ...
    headers: [
      ...
      { title: "Clave Catastral", sortable: false, key: "clave_catastral" },
      { title: "Descripción", sortable: false, key: "descripcion" },
      { title: "Id del Arrendatario", sortable: false, key: "arrId" },
  ...
  methods: {
    initialize() {
      onMounted(async () => {
        const a = await fetch("https://localhost:4000/propiedades", {
          method: "GET",
          headers: {
        ...
        this.datos = pro.map((element) => ({
          id: element.id,
          clave_catastral: element.clave_catastral,
          descripcion: element.descripcion,
          arrId: element.ArrendatarioId,
          Propietarios: element.Propietarios,
        }));
      });
    },
    ...
    deleteItemConfirm() {
      ...
      this.deletePropiedad(this.editedItem);
      this.closeDelete();
    },
    ...
    save() {
      if (this.editedIndex > -1) {
        ...
        const pro = JSON.stringify({
          ...
          ArrendatarioId: this.editedItem.arrId,
        });
        console.log("save " + pro);
        this.updatePropiedad(pro);
      } else {
        ...
        this.postPropiedad(pro);
      }
      this.close();
    },

    async postPropiedad(pro) {
      const res = await fetch("https://localhost:4000/propiedades", {
        method: "POST",
        headers: {
    ...
    async deletePropiedad(pro) {
      const res = await fetch("https://localhost:4000/propiedades/" + pro.id, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${localStorage.getItem("usuario")}`,
        },
